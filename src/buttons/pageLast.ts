...
export default {
    ...
    async execute(interaction: ButtonInteraction) {
        ...
        embed.setFooter({ text: `${queuedTracks.length} canciones  •  Página ${numPages} de ${numPages}` });
        embed.setTimestamp();

        const row1 = new ActionRowBuilder<any>().addComponents(
            new ButtonBuilder().setCustomId(`pageFirst-${interaction.user.id}-${numPages}`).setLabel("Primera página").setStyle(ButtonStyle.Primary),
            new ButtonBuilder().setCustomId(`pagePrevious-${interaction.user.id}-${numPages}`).setLabel("Anterior").setStyle(ButtonStyle.Primary),
            new ButtonBuilder().setCustomId(`pageNext-${interaction.user.id}-${numPages}`).setLabel("Siguiente").setStyle(ButtonStyle.Primary),
            new ButtonBuilder().setCustomId(`pageLast-${interaction.user.id}-${numPages}`).setLabel("Última página").setStyle(ButtonStyle.Primary)
        );
